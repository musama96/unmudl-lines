...
import { CAPTURE_DELAY, STRIPE_SECRET_KEY, TRANSFER_DELAY, twilioClient } from '../config/config';
...
import { CreateGiftEnrollmentDto } from './dto/createGiftEnrollment.dto';
...
import { GiftCourseDto } from '../gift-a-course/dto/giftCourse.dto';
import { RedeemGiftDto } from '../gift-a-course/dto/redeemGift.dto';
...
export class EnrollmentsService {
  ...
  async getApprovedLearnersForCourse(params: CoursePaginationDto) {
  ...
  async createEnrollment(enrollment: CreateEnrollmentDto) {
    ...
    try {
      if (enrollment.promoId && enrollment.discountPercentage === 100) {
        ...
        enrollment.status = course.autoEnroll ? EnrollmentStatus.TRANSFERRED : EnrollmentStatus.PENDING;
        ...
        if (course.autoEnroll) {
          const sendingToCollege =
            enrollment.collegeShare > course.collegeId.payableToUnmudl ? enrollment.collegeShare - course.collegeId.payableToUnmudl : 1;
          enrollment.sentToCollege = sendingToCollege;

          const { data: transfer } = await this.stripeService.createTransferOnConnectAccount(
            course.collegeId.stripeId,
            Math.floor(sendingToCollege * 100),
            transaction.id,
            transferDescription,
          );

          if (course.collegeId.payableToUnmudl > 0) {
            await this.collegesService.updateCollegeOutstandingBalance(
              course.collegeId._id,
              -1 * (enrollment.collegeShare - sendingToCollege),
            );
      ...
      if (course.externalCourseId && course.collegeId.orgId) {
        ...
        try {
          ...
          this.logger.log(stringify(createUserResponse.data));

          // const { data: externalEnrollment } =
          ...
          this.logger.log(stringify(createEnrollmentResponse.data));
        } catch (e) {
          this.logger.log('Error Response:');
          this.logger.log(stringify(e.response ? e.response : e));
          return ResponseHandler.fail(e.response ? e.response.message : e.message);
        }
      }
      ...
      return ResponseHandler.success(newEnrollment, responseMessages.success.enrollmentAdded);
    } catch (e) {
      if (enrollment.deleteCard && enrollment.cardId) {
        await this.stripeService.removeCustomerCard(enrollment.stripeCustomerId, enrollment.cardId);
      }
  ...
  async createEnrollmentForGift(gift, learnerData: LearnerDataDto) {
    const enrollment: CreateGiftEnrollmentDto = {
      courseId: gift.courseId ? gift.courseId : '',
      giftId: gift._id,
      promoId: '', // set promo as gift code id in promo table
    ...
    try {
      ...
      const mailData = {
        ...
        context: {
          unmudlLogo: process.env.UNMUDL_LOGO_PATH,
          date: moment(new Date()).format('LL'),
          gift: { ...gift, totalPaid: gift.totalPaid.toFixed(2) },
          course,
        },
      };
      const mail = await this.mailerService.sendMail(mailData);
      ...
      if (course.externalCourseId && course.collegeId.orgId) {
        this.logger.log(
          `Pragya Call for gift enrollment. External Course ID: (${course.externalCourseId}), OrgId: ${course.collegeId.orgId}.`,
        );
        try {
          const {
            data: { accessToken },
  ...
  async sendMailToCollegeAndLearner(learnerName, course, learner, learnerData, enrollment, isGift = false, sender = null) {
    ...
    if (learnerData.emailAddress) {
      ...
      if (isGift) {
        ...
        if ([EnrollmentStatus.TRANSFERRED, EnrollmentStatus.PROCESSED].includes(enrollment.status)) {
          ...
          mail ? this.emailLogsService.createEmailLog(mailData, Portal.COLLEGE) : null;
        } else {
          enrollment.totalPaid = Number(enrollment.totalPaid.toFixed(2)).toLocaleString();
          const mailData = {
            to: learnerData.emailAddress,
    ...
    if (collegeAdmin && collegeAdmin.length > 0) {
      // console.log(collegeAdmin.emailAddress);
      for (let i = 0; i < collegeAdmin.length; i++) {
        setTimeout(async () => {
          const mailData = {
            to: collegeAdmin[i].emailAddress,
  ...
  async createTransaction(amount, cardId, customerId, description, descriptor, capture = false) {
    ...
    // chargeId => returned from the below transaction
    return await this.stripe.charges.create({
      ...
      statement_descriptor: descriptor, // for customer
      capture,
    });
  }

  async rejectEnrollment(rejectionInfo: RejectEnrollmentDto, enrollmentId, user) {
    ...
    const superadmin = await this.userModel
      .findOne({ collegeId: course.collegeId._id, role: UserRoles.SUPERADMIN }, 'emailAddress')
      .lean()
      .exec();
    ...
    const activities = [
      {
        ...
        transactionActivity: mongoose.Types.ObjectId(
          await this.activitiesService.getTransactionActivityId(TransactionActivities.EnrollmentRefunded),
        ),
      },
    ];
    ...
    return ResponseHandler.success(enrollment, 'Enrollment rejected successfully.');
  }

  async updateLearnerEnrollmentActivityByExternalUser(enrollmentId: string, activity: UpdateLearnerEnrollmentActivityDto) {
    const enrollment = await this.enrollmentModel
      .findByIdAndUpdate(
        enrollmentId,
        {
    ...
    return ResponseHandler.success(enrollment.courseActivity);
  }

  async changeEnrollmentStatus(params: ChangeEnrollmentStatusDto) {
    const enrollment = await this.enrollmentModel
        ...
        if (enrollment.courseFee > 0) {
          try {
            ...
            if (learner.stripeCustomerId || enrollment.giftId || Is100Promo) {
              ...
              if (!Is100Promo) {
                ...
                try {
                  if (course.collegeId.stripeId) {
                    ...
                    return ResponseHandler.fail(
                      `Your college isn't connected to stripe. Please do so, so that we can transfer your payments to your account.`,
                    );
                  }
                } catch (e) {
                  return ResponseHandler.fail(e.response ? e.response.message : e.message ? e.message : e);
              ...
              return ResponseHandler.fail('Card information for this transaction.');
            }
          } catch (e) {
            return ResponseHandler.fail(e.response ? e.response.message : e.message ? e.message : e);
          }
    ...
    // const transferDescription = `Unmudl Transfer: ${course.title}`;
    // if (response.status === EnrollmentStatus.APPROVED) {
    // const { data: capture } = await this.stripeService.capturePaymentFromCustomer(response.transactionId);
    // const { data: transfer } = await this.stripeService.createTransferOnConnectAccount(
    //   course.collegeId.stripeId,
    //   Math.floor(response.collegeShare * 100),
    //   response.transactionId,
    //   transferDescription,
    // );
    //
    // await this.updateTransferStatus(params.enrollmentId, 'transferred', transfer.id, transfer.destination_payment);
    // await this.updateRevenueInfo({
    //   totalRevenue: response.totalRevenue,
    //   unmudlShare: response.unmudlShare,
    //   collegeShare: response.collegeShare,
    //   courseId: response.courseId,
    //   learnerId: response.learnerId,
    // });

    // response.status = EnrollmentStatus.TRANSFERRED;
    // await response.save();
    // }

    if (enrollment.status === EnrollmentStatus.APPROVED || enrollment.status === EnrollmentStatus.DECLINED) {
    ...
    const collegeAdmin =
      enrollment.status === EnrollmentStatus.APPROVED
        ? await this.notificationsService.getUserMails(course.collegeId._id, [UserRoles.SUPERADMIN, UserRoles.ADMIN])
        : [];
    ...
    if (enrollment) {
      return ResponseHandler.success(null, responseMessages.success.changeEnrollmentStatus(params.status));
    } else {
  ...
  async suspendLearnerEnrollments(learnerId, collegeId) {
    ...
    return ResponseHandler.success(enrollments, 'Learner enrollments suspended successfully.');
  }
  ...
  async cancelAllEnrollmentsForCourse(courseId) {
    ...
    await Promise.all(
      enrollments.map(async enrollment => {
        ...
        if (enrollment.status === 'transferred' && enrollment.transferId) {
          // const { data: balanceData } = await this.stripeService.getConnectAccountBalance(enrollment.courseId.collegeId.stripeId);
          // const availableBalance = balanceData.available && balanceData.available.length > 0 ? balanceData.available[0].amount : 0;
          // const pendingBalance = balanceData.pending && balanceData.pending.length > 0 ? balanceData.pending[0].amount : 0;
          ...
          // this.logger.log(`College balance: ${balance}, SentToCollege: ${enrollment.sentToCollege}`);
          // this.logger.log(`Updated outstanding balance: ${outstandingBalance}`);
    ...
    return ResponseHandler.success({ canceledEnrollments: enrollments });
  }
  ...
  async getRevenueAnalytics(params) {
    ...
    return ResponseHandler.success(revenue.length > 0 ? revenue[0] : {});
  }
  ...
  async getCourseDropAnalytics(params) {
    ...
    return ResponseHandler.success(totalEnrollments > 0 ? (droppedEnrollments / totalEnrollments) * 100 : 0);
  }
  ...
  async getLearnersGrowth(params: LearnersSectionAdminDto | GetCountDto) {
    const { collegeId, start, end, interval } = params;
  ...
  async getEnrollmentById(id) {
    ...
    return ResponseHandler.success(enrollment);
  }
  ...
  async refundEnrollment(id, status, transferId = null, courseCanceled = false) {
    ...
    // stripe fee on learner if course was not canceled
    enrollment.totalRevenue = courseCanceled && ['processed', 'transferred'].includes(prevStatus) ? -1 * stripeFee : 0;
    enrollment.unmudlShare = 0;

    // stripe fee on unmudl
    // if (['pending', 'approved'].includes(enrollment.status)) {
    //   enrollment.totalRevenue = 0;
    //   enrollment.unmudlShare = 0;
    // } else {
    //   totalRevenue += enrollment.stripeFee;
    //   unmudlShare += enrollment.stripeFee;
    //   enrollment.totalRevenue = -1 * enrollment.stripeFee;
    //   enrollment.unmudlShare = -1 * enrollment.stripeFee;
    // }

    // didn't know the reason for this block, so commented temporarily
    ...
    // if (transferId) {
    //   enrollment.transferId = transferId;
    // }

    enrollment = await enrollment.save();

    // if enrollment is refunded and its amount was charged while refunding the amount then update revenue info
    if (enrollment.status === 'refunded' && ['processed', 'transferred'].includes(prevStatus)) {
      await this.updateRevenueInfo({
        totalRevenue: -1 * (totalRevenue + (courseCanceled ? stripeFee : 0)), // subtract the original revenue + stripe fee
        unmudlShare: -1 * unmudlShare, // from the revenue fields
  ...
  // @Cron('0 */30 * * * 1-5') // Monday to Friday every 30 minutes
  // @Cron(CronExpression.EVERY_MINUTE)
  async CaptureChargedAmountFromCustomer() {
    const captureDelay = Number(CAPTURE_DELAY);
                    ...
                    1000 * 60 * 60,
                    {
                      $add: [
                        captureDelay,
                        {
                          $switch: {
                            branches: [
                              { case: { $eq: ['$day', 1] }, then: { $subtract: [24, '$hour'] } }, // Sunday
                              {
                                case: { $eq: ['$day', 2] }, // Monday
                                then: {
                                  $cond: {
                                    // if (delay - enrollment day's remaining hours) is greater than the hours remaining till saturday (96 hours)
                                    if: { $gt: [{ $subtract: [captureDelay, { $subtract: [24, '$hour'] }] }, 24 * 4] },
                                    then: 48, // add saturday's and sunday's 48 hours
                                    else: 0,
                                  },
                                },
                              },
                              {
                                case: { $eq: ['$day', 3] }, // Tuesday
                                then: {
                                  $cond: {
                                    // if (delay - enrollment day's remaining hours) is greater than the hours remaining till saturday (72 hours)
                              ...
                              {
                                case: { $eq: ['$day', 4] }, // Wednesday
                                then: {
                                  $cond: {
                                    // if (delay - enrollment day's remaining hours) is greater than the hours remaining till saturday (48 hours)
                              ...
                              {
                                case: { $eq: ['$day', 5] }, // Thursday
                                then: {
                                  $cond: {
                                    // if (delay - enrollment day's remaining hours) is greater than the hours remaining till saturday (24 hours)
                              ...
                              {
                                case: { $eq: ['$day', 6] }, // Friday
                                then: {
                                  $cond: {
                                    // if (delay - enrollment day's remaining hours) is greater than the hours remaining till saturday (0 hours)
                                    // because after today's remaining hours, saturday will start
                              ...
                              { case: { $eq: ['$day', 7] }, then: { $add: [24, { $subtract: [24, '$hour'] }] } }, // Saturday
                            ],
                          },
                        },
                      ],
    ...
    await Promise.all(
      enrollments.map(async enrollment => {
        if (enrollment.transactionId) {
          ...
          // commenting because it can apparently cause bugs
          // await this.updateEnrollmentStatus(enrollment._id, 'transferred');
        }
      }),
    );
      ...
      enrollments.map(async enrollment => {
        try {
          if (enrollment.college && enrollment.college.stripeId) {
            ...
            // const course = await this.courseModel
            //   .findById(enrollment.course._id)
            //   .populate('collegeId')
            //   .lean();
            // const learner = await this.learnerModel.findById(enrollment.learnerId).lean();
            // const collegeAdmin = await this.userModel
            ...
            //         subject: 'UNMUDL Notification',
            //         template: 'enrollmentPaymentTransferStripeMissing',
            //         context: {
            //           unmudlLogo: process.env.UNMUDL_LOGO_PATH,
            //           date: moment(new Date()).format('LL'),
            //           learner,
  ...
  // added the transfer code to capture cron so commenting the transaction cron
  // @Cron('0 */30 * * * 1-5') // Monday to Friday every 30 minutes
  // @Cron(CronExpression.EVERY_MINUTE)
  async TransferPaymentsToCollege() {
    const transferDelay = Number(TRANSFER_DELAY);
        ...
        try {
          if (enrollment.college && enrollment.college.stripeId) {
            if (!enrollment.transferId && !enrollment.destPaymentId) {
              ...
              const { data: transfer } = await this.stripeService.createTransferOnConnectAccount(
                enrollment.college.stripeId,
                Math.floor(enrollment.collegeShare * 100),
                enrollment.transactionId,
                transferDescription,
              );
      ...
      enrollments.map(async enrollment => {
        try {
          ...
          if (enrollment.college.stripeId) {
            ...
            this.logger.log(
              `Transferred $${enrollment.collegeShare.toFixed(2)} to college ${enrollment.college._id} for enrollment
  ...
  async getCourseEnrollmentsCount(courseId) {
    // courseId = mongoose.Types.ObjectId(courseId);
    const status = [EnrollmentStatus.APPROVED, EnrollmentStatus.PENDING, EnrollmentStatus.PROCESSED, EnrollmentStatus.TRANSFERRED];
              ...
              $match: {
                $expr: {
                  $and: [
                    ...
                    { $in: ['$status', [EnrollmentStatus.APPROVED, EnrollmentStatus.PROCESSED, EnrollmentStatus.TRANSFERRED]] },
                  ],
                },
              },
            },
  ...
  async getRecentEnrollmentData(collegeId: string, learnerId: string) {
    const enrollments = await this.enrollmentModel
      .aggregate([
    ...
    return ResponseHandler.success(enrollment.learner);
  }

  async updateLearner(learnerData: LearnerDataDto, learner) {
    if (learnerData.emailAddress) {
    ...
    // learnerData.cumulativePostNineElevenService ? (update.cumulativePostNineElevenService = learnerData.cumulativePostNineElevenService) : (learnerData.cumulativePostNineElevenService = learnerData.cumulativePostNineElevenService);
    // learnerData.completedEnlishment ? (update.completedEnlishment = learnerData.completedEnlishment) : (learnerData.completedEnlishment = learnerData.completedEnlishment);
    // learnerData.isEligiblePostNineElevenBill ? (update.isEligiblePostNineElevenBill = learnerData.isEligiblePostNineElevenBill) : (learnerData.isEligiblePostNineElevenBill = learnerData.isEligiblePostNineElevenBill);
    // learnerData.dependentCount ? (update.dependentCount = learnerData.dependentCount) : (learnerData.dependentCount = learnerData.dependentCount);
  ...
  async getCourseDetails(courseIdDto: CourseNumIdDto, user) {
  ...
  async transferPaymentToCollegeAgainstEnrollment(transferPaymentDto: TransferPaymentDto) {
    const enrollment = await this.enrollmentModel
    ...
    if (!enrollment.courseId.collegeId.stripeId) {
      return ResponseHandler.fail('College stripe id missing.');
    }
    ...
    if (enrollment.sentToCollege === 1 && !enrollment.courseId.collegeId.payableToUnmudl) {
      try {
        ...
        const { data: transfer } = await this.stripeService.createTransferOnConnectAccount(
          enrollment.courseId.collegeId.stripeId,
          Math.floor(transferPaymentDto.amount * 100),
          enrollment.transactionId,
          transferDescription,
        );
      ...
      return ResponseHandler.fail('This enrollment went fine. You cannot modify this enrollment.');
    }
  }
}
