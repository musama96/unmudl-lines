...
export class EmployerEnquiriesService {
  constructor(
    @InjectModel('employer-enquiries') private readonly employerEnquiryModel,
    @InjectModel('employer-enquiry-messages') private readonly employerEnquiryMessagesModel,
  ...
  async addEmployerEnquiryMessage(enquiryData: AddEmployerEnquiryDto) {
    ...
    if (enquiry) {
      ...
      pusher.trigger(
        `employer${collegeId ? '-college' : '-unmudl'}-enquiry-message-${enquiry._id}`,
        `employer${collegeId ? '-college' : '-unmudl'}-new-message`,
        enquiryMessage,
      );
      ...
      admins.forEach(admin => {
        pusher.trigger(
          `new-employer${collegeId ? '-college' : '-unmudl'}-enquiry-group-${admin._id}`,
          `new-employer${collegeId ? '-college' : '-unmudl'}-enquiry-group`,
          newEnquiry,
        );
  ...
  async getEnquiryMessages(params: GetEnquiryMessagesDto) {
    ...
    if (employerAdminId) {
      await this.employerEnquiryMessagesModel.updateMany(
        { _id: { $in: messageIds } },
        { readByEmployerAdmin: true },
  ...
  async addMembers(addMembers: AddEnquiryMembersDto) {
    ...
    const updatedEnquiry = await this.employerEnquiryModel.findOneAndUpdate(
    ...
    return ResponseHandler.success(enquiry, 'Members added successfully.');
  }
  ...
  async updateReadBy(updateMessage) {
    ...
    return ResponseHandler.success(enquiry, 'readBy updated successfully.');
  }
