...
let CollegesService = class CollegesService {
    constructor(collegeModel, collegeTokenModel, enrollmentModel, courseModel, userModel, landingPageModel, counterModel, employerModel, jwtService, unmudlAccessLogsService) {
    ...
    async create(college) {
        ...
        const counter = await this.counterModel
            .findOneAndUpdate({}, { $inc: { college: 1 } }, {
            new: true,
            upsert: true,
        })
            .lean();
        ...
        return ResponseHandler_1.default.success(newCollege);
    }
    ...
    async getCollegesList(params) {
        ...
        return ResponseHandler_1.default.success({ collegesList, collegesCount, coursesCount, employersCount });
    }
    ...
    async getCollegeBasicDetailsByNumId(collegeNumId) {
        return await this.collegeModel
            .findOne({ numId: Number(collegeNumId) }, 'numId collegeLogo collegeLogoThumbnail collegeBanner title createdAt url state city address streetAddress')
            .lean();
    }
    async getColleges(params) {
        ...
        return ResponseHandler_1.default.success({ colleges, rows });
    }
    async getCollegeNamesList(params) {
    ...
    async getCollegeNamesListForEmployerPortal(params, user) {
        const { keyword, page, perPage, sortOrder, sortBy } = params;
        const { subscription: { activePlan, connectedCollege, connectedState }, } = user;
        const sort = {};
        sort[sortBy] = Number(sortOrder);
        const find = { isSuspended: { $ne: true } };
        if (keyword) {
            find.title = {
                $regex: keyword,
                $options: 'i',
    ...
    async getCollegeDetails({ collegeId }) {
        ...
        return ResponseHandler_1.default.success(college.length > 0 ? college[0] : null);
    }
    ...
    async getTopCollegesCsv(params) {
        const { page, perPage, start, end } = params;
        const pipeline = [];
        if (start || end) {
            const createdAt = {};
    ...
    async getTopCollegesRows(params) {
        const { start, end } = params;
        const pipeline = [];
        if (start || end) {
            const createdAt = {};
        ...
        const colleges = await this.collegeModel.aggregate(pipeline).exec();
        return ResponseHandler_1.default.success(colleges.length > 0 ? colleges[0].rows : 0);
    }
    ...
    async updateCollege(college) {
        ...
        const newCollege = await this.collegeModel
            ...
            .exec();
        return ResponseHandler_1.default.success(newCollege, responseMessages_1.default.success.updateCollege);
    }
    async updatePartnerCommission(collegeId, commission) {
        const newCollege = await this.collegeModel
            .findByIdAndUpdate(collegeId, {
            $set: {
            ...
            .lean();
        return ResponseHandler_1.default.success(newCollege, 'Partner commission updated successfully.');
    }
    async updatePartnerGroup(collegeId, partnerGroupId) {
        const newCollege = await this.collegeModel
            ...
            .lean();
        return ResponseHandler_1.default.success(newCollege, 'Partner group updated successfully.');
    }
    ...
    async getCollegeByNumId(collegeId) {
        const college = await this.collegeModel.findOne({ numId: collegeId }).lean();
        if (college) {
            delete college.unmudlShare;
        }
        return college ? ResponseHandler_1.default.success(college) : ResponseHandler_1.default.fail('College not found', null, 404);
    }
    async updateStripeId(collegeId, stripeId) {
    ...
    async checkDomain(emailAddress, collegeId) {
        ...
        if (college && college.domain && !emailAddress.includes(college.domain)) {
            return ResponseHandler_1.default.fail(responseMessages_1.default.inviteUser.invalidDomain);
        }
    }
    ...
    async getCollegeInstructors(collegeNumId, params) {
        ...
        const [instructors, instructorsCount] = await Promise.all([
            this.userModel.aggregate([
                { $match: { collegeId, role: createUser_dto_1.Role.INSTRUCTOR } },
                {
                    $lookup: {
                        from: 'courses',
                        localField: '_id',
            ...
            this.userModel.countDocuments({ collegeId, role: createUser_dto_1.Role.INSTRUCTOR }),
        ]);
            ...
            this.courseModel
                ...
                },
                {
                    $addFields: {
                        sort: {
                            $cond: {
                                if: { $eq: ['$status', courses_model_1.CourseStatus.COMING_SOON] },
                                then: 1,
                                else: 2,
                            },
                        },
    ...
    async getCollegeRevenueGraph(params) {
    ...
    async getCollegeRevenueGraphAsCsv(params) {
        const { collegeId, start, end, interval } = params;
        const match = {
            status: { $in: ['processed', 'transferred'] },
        };
    ...
    async getTransactionActivities(params) {
        ...
        const enrollments = await this.courseModel
            ...
            .limit(perPage)
            .exec();
        return ResponseHandler_1.default.success(enrollments);
    }
    ...
    async suspendUnsupendCollege(collegeId) {
        const college = await this.collegeModel.findById(collegeId).exec();
        ...
        return ResponseHandler_1.default.success({}, `Partner ${college.isSuspended ? 'suspended' : 'unsuspended'} successfully.`);
    }
    ...
    async getCollegesByLocation({ lat, lng }) {
        const colleges = await this.collegeModel
            .find({
            coordinates: {
                $geoWithin: {
    ...
    async getCollegesForEmployerPortal(keyword, user) {
        var _a;
        const { subscription: { activePlan, connectedCollege, connectedState }, } = user;
        const pipeline = [];
        const match = {
            ...
            case 1:
                pipeline.push({
                    $addFields: {
                        isAvailable: {
                            $cond: {
                                if: { $eq: ['$state.shortName', (_a = connectedState) === null || _a === void 0 ? void 0 : _a.shortName] },
                                then: true,
                                else: false,
                            },
                        },
    ...
    async getCollegesByLocationForEmployerPortal({ lat, lng }, user) {
        const { subscription: { activePlan, connectedCollege, connectedState }, } = user;
        const pipeline = [];
        const match = {
            coordinates: {
    ...
    async updateCollegeOutstandingBalance(id, balance) {
        ...
        return ResponseHandler_1.default.success(college, 'College outstanding balance updated successfully.');
    }
    async getUnmudlAdminCollegePortalAccessToken(collegeId, user) {
        ...
        if (!sysAdmin) {
            const counter = await this.counterModel
                .findOneAndUpdate({}, { $inc: { user: 1 } }, {
                new: true,
                upsert: true,
            })
                .lean();
            const newAdmin = {
                fullname: `Unmudl Admin for ${functions_1.default.getInitialsOfWords(college.title)}`,
        ...
        const payload = { _id: sysAdmin._id, emailAddress: sysAdmin.emailAddress, type: 'user' };
...
CollegesService = __decorate([
    ...
    __param(5, mongoose_1.InjectModel('landing-pages')),
    __param(6, mongoose_1.InjectModel('id-counters')),
    __param(7, mongoose_1.InjectModel('employers')),
