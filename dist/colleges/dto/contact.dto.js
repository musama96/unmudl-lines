"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
const openapi = require("@nestjs/swagger");
const class_validator_1 = require("class-validator");
const responseMessages_1 = require("../../config/responseMessages");
const config_1 = require("../../config/config");
class ContactDto {
    static _OPENAPI_METADATA_FACTORY() {
        return { email: { required: false, type: () => String }, number: { required: false, type: () => String }, phoneExtension: { required: false, type: () => String }, name: { required: false, type: () => String } };
    }
}
__decorate([
    class_validator_1.IsOptional(),
    class_validator_1.IsEmail({}, { message: responseMessages_1.default.contact.invalidEmail }),
    __metadata("design:type", String)
], ContactDto.prototype, "email", void 0);
__decorate([
    class_validator_1.IsOptional(),
    class_validator_1.Matches(config_1.PHONE_NUMBER_REGEX, '', { message: responseMessages_1.default.common.invalidPhoneNumber }),
    __metadata("design:type", String)
], ContactDto.prototype, "number", void 0);
__decorate([
    class_validator_1.IsOptional(),
    class_validator_1.IsNumberString(),
    class_validator_1.MaxLength(5),
    __metadata("design:type", String)
], ContactDto.prototype, "phoneExtension", void 0);
__decorate([
    class_validator_1.IsOptional(),
    class_validator_1.IsString({ message: responseMessages_1.default.contact.invalidContactName }),
    __metadata("design:type", String)
], ContactDto.prototype, "name", void 0);
exports.default = ContactDto;
//# sourceMappingURL=contact.dto.js.map