...
let LearnersService = class LearnersService {
    constructor(learnerModel, enrollmentModel, courseModel, notificationsModel, emailLogModel, chatModel, mailerService, coursesService, externalService, learnerTokensService, emailLogsService) {
        ...
        this.learnerTokensService = learnerTokensService;
        this.emailLogsService = emailLogsService;
        this.logger = new common_1.Logger('Learners Service');
    }
    async validateEmail(emailDto) {
        ...
        const learner = await this.learnerModel
            ...
            .lean()
            .exec();
        return learner ? ResponseHandler_1.default.fail(responseMessages_1.default.common.emailRegistered) : ResponseHandler_1.default.success({}, 'Email is valid.');
    }
    async validatePhoneNumber(phoneNumberDto) {
        ...
        return learner
            ...
            : ResponseHandler_1.default.success({}, 'Phone number is valid.');
    }
    async insertLearner(learner) {
        learner.fullname = learner.firstname + ' ' + learner.lastname;
        if (learner.password) {
            learner.password = await functions_1.default.getHash(learner.password);
        }
        else {
            learner.isVerified = true;
        }
        learner.primarySignup = learner.emailAddress ? learner_model_1.PrimarySignup.EMAIL_ADDRESS : learner_model_1.PrimarySignup.PHONE_NUMBER;
        const newLearner = new this.learnerModel(learner);
        const result = await newLearner.save();
        result.password = '';
        return ResponseHandler_1.default.success(result, 'Learner created successfully.');
    }
    ...
    async validateLearnerForLogin(authCredentialsDto) {
        ...
        const learner = await this.learnerModel
            ...
            .select('+password')
            .lean();
        if (learner && (await functions_1.default.compareHash(password, learner.password))) {
            return learner;
        }
        return null;
    }
    ...
    async changePassword(passwordDto, learnerId) {
        const learner = await this.learnerModel
            ...
            .select('+password')
            .exec();
        const verify = await functions_1.default.compareHash(passwordDto.oldPassword, learner.password);
        if (verify) {
            const password = await functions_1.default.getHash(passwordDto.password);
            const updatedLearner = await this.updateLearner({ password }, learnerId);
            return ResponseHandler_1.default.success({}, 'Password changed successfully');
        }
        else {
            ResponseHandler_1.default.fail(responseMessages_1.default.common.incorrectPassword);
        }
    }
    async createPassword(passwordDto, learnerId) {
        ...
        if (!learner.hasPassword) {
            ...
            return ResponseHandler_1.default.success({}, 'Password created successfully');
        }
        else {
            ResponseHandler_1.default.fail(responseMessages_1.default.common.notSocialLogin);
        }
    }
    async sendResetPasswordLink(learner) {
        try {
            const code = await this.learnerTokensService.createLearnerVerification(learner._id.toString());
            const text = 'You are receiving this message because you asked for a password reset. Use this code to reset your password "' + code + '"';
    ...
    async updatePassword(password, learnerId) {
        try {
            const passwordHash = await functions_1.default.getHash(password);
    ...
    async sendVerificationLink(learner) {
        try {
            const code = await this.learnerTokensService.createLearnerVerification(learner._id.toString());
    ...
    async getAnalyticsCountForAdmin(params = { start: null, end: null }) {
    ...
    async getLearnerDetails(params) {
                    ...
                    pipeline: [
                        {
                            $match: {
                                $expr: {
                                    $and: [{ $eq: ['$learnerId', '$$learnerId'] }, { $in: ['$status', [enrollmentStatus_enum_1.EnrollmentStatus.PENDING]] }],
                                },
                            },
                        },
                        { $sort: { createdAt: -1 } },
        ...
        if (learner && learner.length > 0) {
            ...
            if (learner) {
                ...
                return ResponseHandler_1.default.fail('Learner not found.');
            }
        }
    }
    ...
    async blacklistLearner(learnerId) {
        const learner = await this.learnerModel
            ...
            .lean();
        return ResponseHandler_1.default.success(learner);
    }
    async getLearnersEnrollments(learnerId) {
        const [pendingEnrollments, acceptedTempEnrollments, declinedEnrollments] = await Promise.all([
            this.getEnrollments(learnerId, [enrollmentStatus_enum_1.EnrollmentStatus.PENDING]),
            this.getEnrollments(learnerId, [enrollmentStatus_enum_1.EnrollmentStatus.APPROVED, enrollmentStatus_enum_1.EnrollmentStatus.PROCESSED, enrollmentStatus_enum_1.EnrollmentStatus.TRANSFERRED]),
            this.getEnrollments(learnerId, [enrollmentStatus_enum_1.EnrollmentStatus.REFUNDED, enrollmentStatus_enum_1.EnrollmentStatus.CANCELED, enrollmentStatus_enum_1.EnrollmentStatus.DECLINED]),
        ]);
        let accessToken = null;
        const acceptedEnrollments = await Promise.all(acceptedTempEnrollments.map(async (enrollment) => {
            if (enrollment.course.externalCourseId && enrollment.course.college.orgId) {
                ...
                if (accessToken) {
                    const { data: { courseLaunchURL }, } = await this.externalService.getCourseLaunch({
        ...
        return ResponseHandler_1.default.success({ pendingEnrollments, acceptedEnrollments, declinedEnrollments });
    }
    async removeCoursesFromCart(courseIds, learnerId) {
        const learner = await this.learnerModel.findOneAndUpdate({ _id: mongoose.Types.ObjectId(learnerId) }, { $pull: { cart: { course: { $in: courseIds } } } }, { new: true });
        return ResponseHandler_1.default.success({ cartCoursesCount: learner.cart ? learner.cart.length : 0 }, 'Courses successfully removed from cart.');
    }
    async removeCoursesFromWishList(courseIds, learnerId) {
        const learner = await this.learnerModel.findOneAndUpdate({ _id: mongoose.Types.ObjectId(learnerId) }, { $pull: { wishList: { course: { $in: courseIds } } } }, { new: true });
        return ResponseHandler_1.default.success({}, 'Courses successfully removed from wishlist.');
    }
    async getLearnerNotifications(params, learner) {
        ...
        await this.notificationsModel.updateMany({ _id: { $in: notificationIds } }, { isSeen: true }, { multi: true, upsert: false, timestamps: false });
        return ResponseHandler_1.default.success({ notifications, unreadNotificationsCount });
    }
    async getEnrollments(learnerId, status) {
        learnerId = mongoose.Types.ObjectId(learnerId);
    ...
    async updateSuspend(suspendLearner) {
        const learner = await this.learnerModel.findByIdAndUpdate(suspendLearner.learnerId, { isSuspended: suspendLearner.suspend }, { new: true });
        return ResponseHandler_1.default.success({}, `Learner ${learner.isSuspended ? 'suspended' : 'unsuspended'} successfully.`);
    }
    async getLearnerChats(learner, params) {
        const { filter } = params;
        const modules = [];
        if (filter === 'college') {
        ...
        return ResponseHandler_1.default.success(chats);
    }
};
LearnersService = __decorate([
    common_1.Injectable(),
    __param(0, mongoose_1.InjectModel('learners')),
    __param(1, mongoose_1.InjectModel('enrollments')),
    __param(2, mongoose_1.InjectModel('courses')),
    __param(3, mongoose_1.InjectModel('learner-notifications')),
    __param(4, mongoose_1.InjectModel('email-logs')),
    __param(5, mongoose_1.InjectModel('chats')),
    __param(9, common_1.Inject(common_1.forwardRef(() => learnerTokens_service_1.LearnerTokensService))),
    __param(10, common_1.Inject(common_1.forwardRef(() => email_logs_service_1.EmailLogsService))),
